...
\noindent {\bf{HUGH HAN} \vspace{4pt} \\ \large {\bf 600.465 Natural Language Processing} \hfill {{\bf Fall 2016}}}\\
{{\bf Homework \#2}} \hfill {{\bf Due:} 27 September 2016, 5:00pm} \\
...
\item % PROBLEM 5
	...
	\item % b
		\begin{itemize}
			\item[] \hspace{-24pt} 
				\textbf{cross-entropies for the switchboard corpora}
				% TODO : Write up this problem
			\item[] \hspace{-24pt} 
				\textbf{text categorization error rates for gen/spam} \vspace{4pt} \\
				Using $\lambda^* = 0.014$, as calculated in problem \textbf{3}(c), we receive the following classifications for \texttt{gen} and \texttt{spam} test files, respectively.
				\begin{lstlisting}
