...
class LanguageModel:
  def __init__(self):
    ...
    self.XZ, self.YZ = None, None
    self.UXZ, self.VYZ = None, None
    self.Z_xy = None
  ...
  def prob(self, x, y, z):
    ...
    if self.smoother == "UNIFORM":
      ...
      p_zxy = (self.tokens.get((x, y, z), 0) + lambdap_v * p_zy) / (self.tokens.get((x, y), 0) + lambdap_v)
      return p_zxy
      ...
      vec_z = numpy.array(self.vectors[z]).transpose()

      #UX = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
      #VY = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]

      XU = numpy.dot(numpy.array(vec_x), numpy.array(self.U))
      YV = numpy.dot(numpy.array(vec_y), numpy.array(self.V))

      #for j in xrange(0, self.dim):
      #  for m in xrange(0, self.dim):
      #    UX[j][m] = self.U[j][m] * vec_x[j]
      #    VY[j][m] = self.V[j][m] * vec_y[j]
      #    u_xyz += UX[j][m]  * vec_z[m] + VY[j][m] * vec_z[m]

      #u_xyz = math.exp(u_xyz)
      u_xyz = numpy.exp(XU * vec_z + YV * vec_z)[0]
      #print u_xyz
      if (x, y) not in self.Z_xy:
        temp = 0.0
        #u_xyz_ = 0.0
        for z_ in self.vocab:
          ...
          vec_z = numpy.array(self.vectors[z_]).transpose()
          #for j in xrange(0, self.dim):
          #  for m in xrange(0, self.dim):
          #    u_xyz_ += UX[j][m] * vec_z[m] + VY[j][m] * vec_z[m]

          temp += numpy.exp(XU * vec_z + YV * vec_z)[0]

        self.Z_xy[(x, y)] = temp

      return u_xyz / self.Z_xy[(x, y)]
  ...
  def train (self, filename):
    ...
    if self.smoother == 'LOGLINEAR': 
      ...
      for i in range(0, len(tokens_list)):
        if tokens_list[i] not in self.vectors or tokens_list[i] not in self.vocab:
          tokens_list[i] = OOL
      ...
      for i in range(2, len(tokens_list)):
        ...
        xz = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
        yz = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
        uxz = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
        vyz = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
        ...
        for j in range(0, self.dim):
          for m in range(0, self.dim):
            xz[j][m] = vec_x[j] * vec_z[m]
            yz[j][m] = vec_y[j] * vec_z[m]
      ...
      for e in xrange(0, epochs):
        print 'round{0}'.format(e)
        for i in xrange(0, self.N):
          print 'instance{0}'.format(i)
          gamma = gamma0 / (1.0 + gamma0 * (self.lambdap / self.N) * t)

          gradient_U = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
          gradient_V = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]

          for j in xrange(0, self.dim):
            vec_x = self.vectors[f[i][0]]
            vec_y = self.vectors[f[i][1]]

            for m in xrange(0, self.dim):
              gradient_U[j][m] = self.XZ[i][j][m] - (2.0 * self.lambdap / self.N) * self.U[j][m]
              gradient_V[j][m] = self.YZ[i][j][m] - (2.0 * self.lambdap / self.N) * self.V[j][m]

              expect_Ujm = expect_Vjm = 0.0
              for k in xrange(0, self.vocab_size):
                p = self.prob(f[i][0], f[i][1], f[k][2])
                vec_z = self.vectors[f[k][2]]
                expect_Ujm += p * vec_x[j] * vec_z[m]
                expect_Vjm += p * vec_y[j] * vec_z[m]
              
              gradient_U[j][m] += expect_Ujm
              gradient_V[j][m] += expect_Vjm

          for j in xrange(0, self.dim):
            for m in xrange(0, self.dim):
              self.U[j][m] += gamma * gradient_U[j][m]
              self.V[j][m] += gamma * gradient_V[j][m]
