...
\item % PROBLEM 6
	...
	\item % g
		...
		\item % iv
			...
			For the \texttt{english} test files, the cross-entropy increased tremendously while for the \texttt{spanish} test files, the cross-entropy increased only slightly from before this feature was implemented.

			Below are our error-rate results for the \texttt{english} and \texttt{spanish} test files, respectively.
			\begin{lstlisting}
		...
		316 looked more like sp.1K (85.64%)
			...
			We see that our error rates are much better for \texttt{spanish}, but worse for \texttt{english}. Note that these tests were run using $\gamma_0 = 0.01$.
		\end{enumerate}
	\end{enumerate}
\item % PROBLEM 7
