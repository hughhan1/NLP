...
\item 
	For this part, we just implemented some easy speedup tricks. First, we handled the categories have already been predicted for the current column. 

	That is, we keep track of them. If we're about to predict a batch of several hundred \texttt{NP} rules (all rules of the form \texttt{NP} $\to$ \texttt{. BLAH BLAH}), then our program first does a quick check to discover whether we've already added that batch to the current column.
	...
	Another thing we implemented was 1-lookahead.
